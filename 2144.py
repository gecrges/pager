...
def update_and_draw_cd(
    ...
    rotated_cd = pygame.transform.rotate(
        CD_SURFACE,
        cd_angle
    )
...
def monochrome_cover_fit(
    ...
    src_pixels = pygame.PixelArray(cropped)
    mono_pixels = pygame.PixelArray(mono)

    for y in range(target_h):

        for x in range(target_w):

            color = cropped.unmap_rgb(
                src_pixels[x, y]
            )
